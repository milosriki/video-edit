...
// FIX: Cast app to any to avoid "No overload matches this call" error when using middleware like cors or express.json
...
// FIX: Use 'any' for res to avoid type errors with Express's json/status methods in this environment.
app.get('/api/avatars', (_req: any, res: any) => {
  try {
...
// FIX: Use 'any' for req/res to resolve missing property errors (body, json, status).
...
// FIX: Use 'any' for req/res to resolve missing property errors (body, status, json).
app.post('/api/creatives', async (req: any, res: any) => {
  try {
...
// FIX: Use 'any' for req/res to resolve missing property errors (body, status, json).
app.post('/api/creatives/rank', async (req: any, res: any) => {
  try {
    const { creatives, avatarKey, brief } = req.body as {
...
// FIX: Use 'any' for req/res to resolve missing property errors (query, status, json).
app.get('/api/overview', (req: any, res: any) => {
...
// FIX: Use 'any' for req/res to resolve missing property errors (query, status, json).
app.get('/api/creatives', (req: any, res: any) => {
    ...
    results.sort((a, b) => {
        ...
        return safeOrder === 'ASC' ? valA - valB : valB - a[safeSort as keyof typeof a];
    });
...
// FIX: Use 'any' for req/res to resolve missing property errors (query, status, json).
app.get('/api/timeseries', (req: any, res: any) => {
...
// FIX: Use 'any' for req/res to resolve missing property errors (body, write, json).
...
// FIX: Use 'any' for req/res to resolve missing property errors (setHeader, flushHeaders, write, on).
