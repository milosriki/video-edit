...
const AdvancedEditor: React.FC<AdvancedEditorProps> = ({ sourceVideo, onClose }) => {
  ...
  // FIX: Explicitly typing 'newEdit' as 'AdvancedEdit' within each case ensures the correct discriminated union variant is inferred, resolving the state update type error.
  const addEdit = (type: AdvancedEdit['type']) => {
    setEdits((prev) => {
      ...
      switch (type) {
        case 'trim': {
          const newEdit: AdvancedEdit = { id, type: 'trim', start: '0.00', end: '5.00' };
          return [newEdit, ...prev.filter((e) => e.type !== 'trim')];
        }
        case 'text': {
          const newEdit: AdvancedEdit = { id, type: 'text', text: 'Hello World', start: '0.00', end: '3.00', position: 'center', fontSize: 48 };
          return [...prev, newEdit];
        }
        case 'image': {
          const placeholderFile = new File([], 'placeholder.png', { type: 'image/png' });
          const newEdit: AdvancedEdit = { id, type: 'image', file: placeholderFile, position: 'top_right', scale: 0.2, opacity: 1.0 };
          return [...prev, newEdit];
        }
        case 'speed': {
          const newEdit: AdvancedEdit = { id, type: 'speed', factor: 2.0 };
          return [...prev, newEdit];
        }
        case 'filter': {
          const newEdit: AdvancedEdit = { id, type: 'filter', name: 'grayscale' };
          return [...prev, newEdit];
        }
        case 'mute': {
          const newEdit: AdvancedEdit = { id, type: 'mute' };
          return [...prev, newEdit];
        }
        default:
          return prev;
  ...
  const updateEdit = (id: string, newValues: Partial<AdvancedEdit>) => {
    // FIX: Type assertion used to handle the complexity of updating partial properties on a discriminated union.
    setEdits(prev => prev.map(e => e.id === id ? { ...e, ...newValues } as AdvancedEdit : e));
  };
